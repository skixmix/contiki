...
typedef enum cn_cbor_error {
  CN_CBOR_NO_ERROR = 0,
  CN_CBOR_ERR_OUT_OF_DATA = 1,
  CN_CBOR_ERR_NOT_ALL_DATA_CONSUMED = 2,
  CN_CBOR_ERR_ODD_SIZE_INDEF_MAP = 3,
  CN_CBOR_ERR_BREAK_OUTSIDE_INDEF = 4,
  CN_CBOR_ERR_MT_UNDEF_FOR_INDEF = 5,
  CN_CBOR_ERR_RESERVED_AI = 6,
  CN_CBOR_ERR_WRONG_NESTING_IN_INDEF_STRING = 7,
  CN_CBOR_ERR_OUT_OF_MEMORY = 8,
  CN_CBOR_ERR_FLOAT_NOT_SUPPORTED = 9,
} cn_cbor_error;
...
uint8_t cn_cbor_encode_uint(char* buf, void* value, uint8_t size);
uint8_t cn_cbor_encode_int(char* buf, void* value, uint8_t size, uint8_t sign);
