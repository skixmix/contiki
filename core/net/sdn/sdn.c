...
sdn_init(void)
{
    ...
    rpl_config = RPL_BYPASS; //Specifying to bypass if rpl
...
int forward(){
    ...
    PRINTF("SDN: The packet is being sent to: ");
    PRINTLLADDR(dest);
    PRINTF("\n");
#endif
    ...
    printf("MSG_SENT_BYTES_SDN_%u\n", packetbuf_totlen());
		...
		if(pktHasMeshHeader(packetbuf_dataptr()) == 1){
			...
			if(hopLimit == MESH_TAG_CONTROL_HL)
				PRINT_STAT("\nS_FC_%u\n", packetbuf_datalen()); //SDN control
		else if(hopLimit == MESH_TAG_RPL_HL)
			PRINT_STAT("\nS_FR_%u\n", packetbuf_datalen()); //RPL control
			else
				PRINT_STAT("\nS_FD_%u\n", packetbuf_datalen());
		}
		else
...
input(void)
{
    ...
    PRINTF("SDN: A packet has been received from: ");
    ...
    if(pktHasMeshHeader(ptr) == 1){
        ...
        if(meshAddrIsMulticast(&finalAddr, finalAddrDim) == 1){
            PRINTF("SDN: it is a multicast packet\n");
            ...
            if(meshAddrListContains(&finalAddr) == 1){
                PRINTF("SDN: sending multicast packet to upper layer\n");
            ...
            if(meshAddrListContains(&finalAddr) == 1){
                //This packet must be sent to the upper layer
                PRINTF("It is for me, packet forwarded to 6LoWPAN layer\n");
                NETSTACK_NETWORK.input();
            }
            else{
                //Otherwise it must be handled by the Flow Table
				PRINTF("It is NOT for me, checking flow table\n");
                processPacket();
            }
        }
    }
		...
		PRINTF("Packet does not have mesh header, reading destination from ipv6 header\n");
        ...
        if(isRPLMulticast(&ipAddr) == 1){
            ...
            if(rpl_config == RPL_BYPASS){       
				...
				PRINTF("Not RPL_BYPASS, Handling with flow table\n");
                processPacket();
            }
        }
        else{
            ...
            if(uip_is_addr_mcast(&ipAddr) == 1){
				printf("It is a MULTICAST packet\n");
                ...
                */
                ...
                if(ipAddrListContains(&ipAddr) == 1){
                    //This packet must be sent to the upper layer
					printf("IPv6 address is addressed to me or to the controller, passing to upper layer\n");
                    NETSTACK_NETWORK.input();
                }
                else{
                    //Otherwise it must be handled by the Flow Table
					printf("Not a RPL packet and not for the controller, handling with flow table\n");
                    processPacket();
                }
            }
        }    
...
send(mac_callback_t sent, void *ptr)
{
    ...
    uint8_t origAddrDim;
    dest = packetbuf_addr(PACKETBUF_ADDR_RECEIVER);
    PRINTF("SDN: 6LoWPAN layer -> Packet is being sent to: ");
    PRINTLLADDR(dest);
    PRINTF("\n");
